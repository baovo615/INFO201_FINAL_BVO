...
base.uri1 <- "https://www.haloapi.com/stats/h5/player-leaderboards/csr/"
# resource {seasonId}/{playlistId}
resource.uri1 <- paste0(latest.finished.season.id, "/c98949ae-60a8-43dc-85d7-0feb0b92e719?count=250")
uri1 <- paste0(base.uri, resource.uri)
response1 <- GET(uri, add_headers('Ocp-Apim-Subscription-Key' = api.key))
response1 <- fromJSON(content(response, "text"))
leaders1 <- response$Results %>% flatten()

rows.to.used1 <- seq(10, 169, 10)
rows.to.used1 <- c(1, rows.to.used)

condense.leaders1 <- leaders[rows.to.used, ]
gametags1 <- condense.leaders[["Player.Gamertag" ]]

#This will add a '+' instead of a space in the URL for the
#GetMatchID function
gametags1 <- gsub('([[:punct:]])|\\s+','+',gametags)
...
read.csv("data/killdeathratio.csv", stringsAsFactors = FALSE)
